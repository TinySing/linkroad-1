<template>
  ...
  <div class="person_main">
    ...
    <div class="top_area">
      <!--搜索框部分-->
      <el-input @input="change($event)" v-model="searchData" class="search" type="text" placeholder="姓名/用户名"
                @keyup.enter.native="searchBut"></el-input>
      <div class="but">
            ...
            <el-dialog title="人员相关系统"
              ...
              <div class="diaBox"
                ...
                >
                  ...
                  >
                    <template slot-scope="scope">
                      <div style="cursor: pointer" @click="chooseDuty(scope.$index,tableDataTest,tableData)">
                  ...
                  >
                    <template slot-scope="scope">
                      <div style="cursor: pointer" @click="chooseJoin(scope.$index,tableDataTest,tableData)">
                        <el-button type="text" size="small">
                          添加参与系统
                        </el-button>
                      </div>
    ...
    methods: {
      ...
      getSystem() {
        ...
        let information = {
          ...
          "projectInfo": this.searchDataTest,
        };
        ...
        this.$axios.post('/monitorCon/system/project/queryCascadeProject', qs.stringify(information)).then((response) => {
          ...
          for (let i = 0; i < response.data.list.length; i++) {
            this.tableDataTest.push({
              'projectname': response.data.list[i].projectname,
              'projectid': response.data.list[i].projectid,
            })
          }
        });
      },
      ...
      handleDialogClose() {
        this.centerDialogTest = false
        this.searchDataTest = ''
      },
      chooseDuty(index, tableDataTest, tableData) {
        this.userId = tableData[this.importantIndex].userId;
        this.systemId = tableDataTest[index].projectid;
        ...
        this.$axios.post('/monitorCon/system/userProject/add', qs.stringify(information)).then((response) => {
          ...
          if (response.data.type === 'fail') {
            this.$message.error('添加失败，此人已是该项目负责人！！！');
          }
          this.getSystem()
          // this.centerDialogTest = false
        });
      },
      chooseJoin(index, tableDataTest, tableData) {
        this.userId = tableData[this.importantIndex].userId;
        this.systemId = tableDataTest[index].projectid;
